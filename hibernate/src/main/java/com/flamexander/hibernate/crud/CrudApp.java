...
public class CrudApp {
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            session.beginTransaction();
            maxId = session
                    .createQuery("SELECT MAX(s.id) FROM SimpleItem s", Long.class)
                    .getSingleResult();
            SimpleItem simpleItemForUpdate = session
                    .createQuery("SELECT s FROM SimpleItem s WHERE s.id = :id", SimpleItem.class)
